...
public class DisruptorBuilder<T> {
    ...
    private ThreadFactory threadFactory = new ThreadFactoryBuilder().setNameFormat("Disruptor-%d").build();
    ...
    public Disruptor<T> build() {
        Assert.notNull(this.ringBufferSize, " Ring buffer size not set");
        Assert.notNull(this.eventFactory, "Event factory not set");
